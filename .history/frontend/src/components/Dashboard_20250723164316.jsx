...
// Enhanced mock devices: removed profileImg as requested
const mockDevices = [
  { id: 1, name: "Raiyan's iPhone", type: "mobile", status: "online", user: "Raiyan Islam", ip: "192.168.1.10", blocked: false },
  { id: 2, "name": "Al's Gaming PC", type: "desktop", status: "online", user: "Al Imam Uddin", ip: "192.168.1.12", blocked: false },
  { id: 3, name: "Mihi's iPad Pro", type: "tablet", status: "offline", user: "Rukaiya Jahan", ip: "192.168.1.15", blocked: true },
  { id: 4, name: "Living Room TV", type: "tv", status: "online", user: "Family", ip: "192.168.1.20", blocked: false }
];

// Re-purposed mockRealtimeActivity to represent current Browse for online devices
const mockRealtimeActivity = [
  { deviceId: 1, site: "youtube.com/watch?v=some_video", deviceName: "Raiyan's iPhone", timestamp: new Date() },
  { deviceId: 2, site: "discord.com/chat/general", deviceName: "Al's Gaming PC", timestamp: new Date() },
  { deviceId: 4, site: "twitch.tv/streamer_xyz", deviceName: "Living Room TV", timestamp: new Date() },
];

// Mock historical Browse data (kept separate for full history)
const mockHistoricalActivity = [
...
// Mock historical Browse data for a specific device based on filter
const mockHistoricalBrowse = (deviceId, filter) => {
  ...
  const filteredLogs = mockHistoricalActivity.filter(log =>
...
const Dashboard = () => {
  ...
  // Using mockRealtimeActivity for current activity display
  ...
  const fetchData = useCallback(() => {
      ...
      setTimeout(() => {
          ...
          setCurrentActivityLog(mockRealtimeActivity); // Using mockRealtimeActivity for current display
      ...
      <div className="dashboard-container">
          ...
          <main className="dashboard-main">
              ...
              <div className="main-content-grid">
                          ...
                          {devices.map(device => (
                              <div key={device.id} className={`device-card ${device.blocked ? 'blocked' : ''}`}>
                                  <div className="device-header">
                                      {/* Removed img tag for profileImg */}
                  ...
                  {/* New Current Device Activity Section (Real-time activity logs) */}
                  <section className="content-section current-activity-section">
                      <div className="section-header">
                          <h2>Current Device Activity ({currentActivityLog.length} Online)</h2>
                      </div>
                      <ul className="current-activity-log">
                          {currentActivityLog.length > 0 ? (
                              currentActivityLog.map((log, index) => (
                                  <li key={index}>
                                      <span className="device-name">{log.deviceName}</span>
                                      <span className="current-site">Browse: <a href={`http://${log.site}`} target="_blank" rel="noopener noreferrer">{log.site}</a></span>
                                      <span className="log-time">Last seen: {formatTime(log.timestamp)}</span>
                                  </li>
                              ))
                          ) : (
                              <li>No active Browse sessions.</li>
                          )}
                      </ul>
                  </section>

                  {/* Weekly Usage Reports Section */}
                  <section className="content-section usage-reports-section">
                      <div className="section-header">
