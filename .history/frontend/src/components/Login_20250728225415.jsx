...
const Login = () => {
 ...
 const handleRegister = async (e) => {
  ...
  // Create a FormData object to handle the multipart (file + text) request
  ...
  // The file input is not directly in state, so we access it via the ref
  if (fileInputRef.current.files[0]) {
    formData.append('profileImage', fileInputRef.current.files[0]);
  }

  try {
    // Send the POST request to your backend registration endpoint
    ...
    // On success, alert the user and switch to the login form
